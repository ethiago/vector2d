...
*  in a way that one can iterate the entire data, or a specifica index dimesion
*/
...
struct vector2D
{
    ...
    using jump_iterator = nstd::jump_iterator<iterator>;
    using const_jump_iterator = nstd::jump_iterator<const_iterator>;

    using range = nstd::range<const_jump_iterator>;
    ...
    auto slice(std::size_t idx) const noexcept
    {
        ...
        if constexpr (dim == 0)
        {
          ...
          return nstd::range{bg, bg+_n[1]};
        }else
        {
          auto bg = const_jump_iterator(_container.cbegin() + idx, _n[1]);
          return nstd::range{bg, bg+_n[0]};
        }          
    }
